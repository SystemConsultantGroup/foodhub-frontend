 ...
 */
const ProfileImage: React.FC<Props> = ({ size = 30 }: Props) => {
  const userImageSrc = null; // 유저가 프로필 세팅했을 시
  const defaultPhotoId = 1; // 유저가 프로필 세팅하지 않았을 시 기본 제공 값 (API 에서 받아올 예정)
  const imageSrc = userImageSrc ?? `/images/defaults/profile/${defaultPhotoId}.svg`;

  return (
    <EmotionWrapper size={size}>
...
const EmotionWrapper = styled.div<Required<Props>>`
  img {
